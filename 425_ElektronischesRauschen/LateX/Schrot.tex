...
Schrotrauschen entsteht aufgrund der zufälligen statistischen Fluktuation diskreter elektrischer Ladungsträger. Es tritt auf, sobald unkorrelierte Elektronen eine Potentialbarriere (z.B. Photodiode) überwinden. Dieser Prozess ist statistischer Natur und folgt der Poisson-Verteilung.\\
In diesem Versuchsteil wird ein solcher Elektronenstrom durch den Photoeffekt erzeugt. Eine Glühlampe emittiert diskrete Photonen, welche auf eine Photodiode treffen und dort Elektronen rauslösen, welche wiederum Stromfluktuationen erzeugen.\\
Diese lassen sich über die Schottky-Formel quantifizieren:
\begin{equation}
    \overline{\delta_i^2} = 2ei_{\mathrm{dc}}\Delta f\label{equ:schottky}
\end{equation}
...
\end{figure}
Die Photodiode ist über einen invertierenden Verstärker mit Rückkopplungswiderstand $R_f = \qty{10}{\kilo\ohm}$ in der LLE-Box verschaltet. Der erzeugte Photostrom liefert eine proportionale Spannung:
\begin{equation}
...
\end{equation}
Zur Messung des Schrotrauschens wird der Tiefpass der HLE-Box auf eine Grenzfreuqenz $f_{\mathrm{gr}} = \qty{100}{\kilo\hertz}$ und alle Schalter auf AC eingestellt. Die Verstärkung $G_2$ wird so gewählt, dass das Signal $\overline{V_{\mathrm{Meter}}}$ Werte im Bereich von \SIrange{0.6}{1.2}{\volt} anzeigt, um im linearen Bereich zu bleiben.\\
Das Rauschen wurde nun auf einem Oszilloskop sichtbar gemacht (\cref{fig:schrot_oszi}).
\begin{figure}[h]
    \centering
    \includegraphics[keepaspectratio, width=0.6\linewidth]{images/IMG_0956.png}
    \caption{Darstellung des Schrotrauschens auf dem Oszilloskop $V$/div $=\qty{5}{\volt}$; $T$/div $=\qty{4}{\micro\second}$}\label{fig:schrot_oszi}
\end{figure}
% ----
...
% ----
\section[Schrotrauschen (Gleichstrom)]{Abhängigkeit des Schrotrauschens vom Gleichstrom}
% ----
...
\begin{equation}
    \overline{\delta_i^2} = \frac{\overline{V_{\mathrm{Meter}}(t)}\cdot \qty{10}{\volt}}{(100\cdot G_2 R_f)^2} - \overline{\delta_{i, \mathrm{offset}}^2},\quad\Delta\overline{\delta_i^2} = \sqrt{\left(\frac{\overline{\Delta V_{\mathrm{Meter}}(t)}\cdot \qty{10}{\volt}}{(100\cdot G_2 R_f)^2}\right)^2 - \left(\Delta\overline{\delta_{i, \mathrm{offset}}^2}\right)^2}\label{equ:schrot_delta}
\end{equation}
...
\end{itemize}
was für ein zu gutes Anpassungsmodell spricht. Dies lässt sich durch Betrachtung der (normierten) Residuen bestätigen, da diese nah um Null herum verteilt sind. Hier wurden wahrscheinlich die Unsicherheiten überschätzt, was dazu führt, dass die Anpassung statistisch zu gut ist.\\
% ----
\section[Schrotrauschen (Bandbreite)]{Abhängigkeit des Schrotrauschens von der Bandbreite}
% ---
Um die Abhängigkeit des Schrotrauschens von der Bandbreite und somit seine \enquote{Weißheit}(also konstante Rauschleistung über die Bandbreite) zu untersuchen, wird nun der Gleichstrom $i_\mathrm{dc}$ konstant gehalten, indem $V_{\mathrm{Monitor}}=\qty{10.02\pm 0.05}{\volt}$ eingestellt wurde. Die Grenzfrequenz des Tiefpasses $f_l$ wird nun variiert, wobei geachtet wurde, dass die Verstärkung $G_2$ eine Ausgangsspannung der HLE-Box zwischen $\SIrange{0.6}{1.2}{\volt}$ ermöglicht. Der Widerstand der LLE-Box wurde auf $R_f = \qty{10}{\kilo\ohm}$ eingestellt und die Werte für die mittlere Stromschwankung gemäß \cref{equ:schrot_delta} ermittelt (\cref{tab:schrot_bandbreite}).
\begin{table}[h!]
    \centering
    \begin{tabular}{
        S[table-format=6.0]
...
\end{table}
Nun wird das Stromschwankungsquadrat gegen die effektive Bandbreite aufgetragen und an die Messwerte eine lineare Anpassung der Form $y(x)=mx+b$ durchgeführt (\cref{fig:schrot_bandbreite}).
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{plots/schrot_rauschen_bandbreite.pdf}
    \caption{Darstellung der mittleren quadratischen Stromschwankungen in Abhängigkeit von der Bandbreite}\label{fig:schrot_bandbreite}
\end{figure}
Das Modell liefert:
\begin{itemize}
    \item $m = \qty{3.53(10)e-22}{\ampere\squared\per\hertz}$
    \item $b = \qty{-3.37(9)e-19}{\ampere\squared}$
    \item $\chi_{\mathrm{red}}^2 = 0.8$
\end{itemize}
was für eine gelungene Anpassung spricht.
% ----
\section{Berechnung der Elementarladung}
% ----
Nun kann gemäß der Schottky-Gleichung \cref{equ:schottky} die Elementarladung $e$ anhand der ermittelten Steigungen der Stromschwankungsquadrate berechnet werden:
\begin{align}
    \overline{\delta_i^2} &= 2ei_{\mathrm{dc}}\Delta f\\
    \Rightarrow e_{\exp, i} &= \frac{\overline{\delta_i^2}}{2i_{\mathrm{dc}}\Delta f} = \left\{
  \begin{array}{@{}l@{\quad}l@{}}
    \frac{m}{2\Delta f} & \text{,für die Gleichstrommessung}\\[4pt]
    \frac{m}{2i_\mathrm{dc}} & \text{,für die Bandbreitenmessung}\\[4pt]
  \end{array}
  \right.
\end{align}
Die effektive Bandbreite für den Gleichstrom ergibt sich aus dem Frequenzintegral:
\begin{eqnarray}
    \Delta f = \int_{0}^{\infty} \frac{1}{1+\left(\frac{f}{f_l}\right)^4} = \qty{111.07}{\kilo\hertz}
\end{eqnarray}
Aus den Messwerten ergeben sich folgende experimentelle Werte für die Elementarladung:
\begin{align*}
    &\text{Gleichstrommessung:  } e_{\exp,\mathrm{strom}} = \qty{1.89(40)e-19}{\coulomb}\\
    &\text{Bandbreitenmessung:  } e_{\exp,\mathrm{freq}} = \qty{1.75(5)e-19}{\coulomb}
\end{align*}
Die relativen Abweichungen zum Literaturwert von $e_{\mathrm{lit}}= \qty{1.602176634e-19}{\coulomb}$\cite{NIST} sowie die $\sigma$-Abweichungen sind in \cref{tab:schrot_elementar} festgehalten.
\begin{table}[h]
    \begin{tabular}{
        c|
        S[table-format=1.2(2)]|
    ...
    \toprule
    \multicolumn{1}{c|}{Methode} &
    \multicolumn{1}{c|}{$e$ / $10^{-19}$ C} &
    \multicolumn{1}{c|}{Abweichung rel. zum Literaturwert / \%} &
    \multicolumn{1}{c}{$\sigma$-Abweichung}
    \\
    \midrule
    $i_{\mathrm{dc}}$ variiert & 1.89 \pm 0.4 & 17.9 & 0.7 \\
    $\Delta f$ variiert & 1.75 \pm 0.05 & 7.9 & 3.0\\
    \bottomrule
    \end{tabular}
    \caption{Ermittelte Werte der Elementarladung $e$}\label{tab:schrot_elementar}
\end{table}
Die $\sigma$-Abweichung für die Gleichstrommessung ist bei $1\sigma$, was für gut geschätzte Fehler spricht, wobei die der Bandbreitenmessung mit $3\sigma$ für zu klein geschätzte Fehler sprechen. Beide Werte liegen jedoch nah bis sehr nah am Literaturwert für die Elementarladung, was eine gelungene Durchführung und Auswertung steht.