...
\begin{figure}[ht]
    ...
    \label{fig:pickup}
\end{figure}
Hierzu wurde eine RF Pickup Probe (\cref{fig:pickup}) in den Messkopf des Magneten eingesetzt, und an den Eingang des Oszilloskops angeschlossen. Da hier nur ein einzelner Puls betrachtet wird wurde der Puls B am Mainframe ausgeschaltet und die Länge des A-Pulses auf $A_{\mathrm{len}}=\qty{3}{\micro\second}$ und die Periodendauer auf $P=\qty{100}{\milli\second}$ eingestellt und auf dem Oszilloskop das Signal des Pickup-Sensors betrachtet.
\begin{figure}[ht]
    \centering
...
%------
Nun wird die Pickup Probe entfernt und die Probe mit Mineralöl eingesetzt, um das FID-Signal (Free Induction Decay) zu betrachten, welches nach einem einzelnen A-Puls als \enquote{Antwort der Probe}~\cite{praktikum4_atome} auftritt. Zunächst werden die vier Potentiometer am PS2 Controller ($X,Y,Z,Z^2$) auf Null eingestellt. Man erwartet nun ein Signal, welches einem exponentiellen Abfall ähnelt, da die Magnetisierung in der $xy$-Ebene nach dem $\frac{\pi}{2}$-Puls aufgrund der transversalen Relaxation mit der Zeit abfällt.
Die Dynamik lässt sich anschaulich auf der \textit{Bloch-Kugel} (\cref{fig:spinecho_bloch} a bis c) beschreiben. Nach dem $\frac{\pi}{2}$-Puls liegt die Magnetisierung in der $xy$-Ebene und präzediert um das statische Magnetfeld $B_0$. Aufgrund von Inhomogenitäten im Magnetfeld dephasieren die einzelnen Spins jedoch mit der Zeit, was zu einem Abfall der Gesamtmagnetisierung in der $xy$-Ebene führt.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\textwidth]{figs/teachspin_spinecho_bloch.png}
    \caption{Blochkugel im rotierenden Bezugssystem.\\ \textbf{a} Grundzustand der Magnetisierung entlang $z$.\\ \textbf{b} Nach einem $\pi/2$-Puls liegt die Magnetisierung in der $xy$-Ebene.\\ \textbf{c} Die Magnetisierungen dephasieren in unterschiedliche Richtungen über einen Zeitraum $\tau$\\ \textbf{d} Nach einem $\pi$-Puls drehen sich die Magnetisierungen um und präzedieren weiter.\\ \textbf{e} Nach weiterer Zeit $\tau$ haben sich die Magnetisierungen wieder in Phase und es entsteht ein Spin-Echo.\cite{teachspin}}
    \label{fig:spinecho_bloch}
\end{figure} 
Durch Justage der Potentiometer und der Frequenz am Synthesizer wird versucht, dieses Signal zu optimieren. Ein optimiertes FID-Signal ist in~\cref{fig:fid_signal} dargestellt. Hierbei ist eine Zerfallszeit von etwas über $\qty{4}{\milli\second}$ zu erkennen, welche durch den eingezeichneten \enquote{1/e Abfall} markiert ist.\\
...
\end{figure}
%------
\subsection{$\frac{\pi}{2}$- und $\pi$-Puls Justage}\label{sec:aufbau_puls}
%------
%chktex-file 35
